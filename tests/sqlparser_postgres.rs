...
fn parse_alter_table_alter_column_add_generated() {
    ...
    // pg_dump spells an identity column's backing sequence with SEQUENCE NAME.
    pg_and_generic().verified_stmt(
        "ALTER TABLE t ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY (SEQUENCE NAME s.t_id_seq START WITH 100 INCREMENT BY 5 NO MINVALUE NO MAXVALUE CACHE 1)",
    );
    pg_and_generic().verified_stmt(
