...
use super::common::OracleCase;
use super::negative::NEGATIVE_CASES;
use super::plsql::PLSQL_CASES;
use super::relational::RELATIONAL_CASES;
use super::statements::STATEMENT_CASES;

struct GrammarObligation {
    production: &'static str,
    scope: GrammarScope,
    positive_cases: &'static [&'static str],
    negative_cases: &'static [&'static str],
}
...
const GRAMMAR_OBLIGATIONS: &[GrammarObligation] = &[
    GrammarObligation {
...
fn all_positive_ids() -> BTreeSet<&'static str> {
    RELATIONAL_CASES
        .iter()
        .chain(PLSQL_CASES)
        .chain(STATEMENT_CASES)
        .map(|case: &OracleCase| case.id)
        .collect()
}

fn positive_case(id: &str) -> Option<&'static OracleCase> {
    RELATIONAL_CASES
        .iter()
        .chain(PLSQL_CASES)
        .chain(STATEMENT_CASES)
...
fn grammar_obligations_reference_existing_cases() {
    let positives = all_positive_ids();
