...
fn parse_is_null() {
    ...
    assert!(matches!(
        verified_expr(sql),
        Expr::IsNull { expr, .. } if matches!(*expr, Expr::Identifier(_))
    ));
}
...
fn parse_is_not_null() {
    ...
    assert!(matches!(
        verified_expr(sql),
        Expr::IsNotNull { expr, .. } if matches!(*expr, Expr::Identifier(_))
    ));
}
...
fn parse_is_distinct_from_precedence() {
    use self::Expr::*;
    // AND and OR bind looser than IS: the predicate ends before them.
    assert_eq!(
        BinaryOp {
        ...
        verified_expr("a IS DISTINCT FROM b AND c")
    );
    assert_eq!(
        BinaryOp {
        ...
        verified_expr("a IS NOT DISTINCT FROM b OR c")
    );
    // Comparison binds tighter than IS: the right operand is `b = c`.
    assert_eq!(
        IsDistinctFrom(
            Box::new(Identifier(Ident::new("a"))),
            Box::new(BinaryOp {
...
// parse_group_by_with_modifier: removed (no remaining dialects support supports_group_by_with_modifier)
...
// parse_window_function_null_treatment_arg: removed (no remaining dialects support supports_window_function_null_treatment_arg)
...
fn parse_create_table_as() {
    ...
    // Note: BigQuery allows specifying table schema in CTAS like:
    // CREATE TABLE t (a INT, b INT) AS SELECT 1 AS b, 2 AS a
    // but BigQuery is not in the tested dialects, so we don't test that here
}
...
fn parse_alter_table_drop_column() {
    ...
    // Note: Snowflake supports comma-separated drop column lists like:
    // ALTER TABLE tbl DROP COLUMN c1, c2, c3
    // but Snowflake is not in the tested dialects, so we don't test that here
...
fn parse_exists_subquery() {
    ...
    let res = all_dialects().parse_sql_statements("SELECT EXISTS (");
    assert_eq!(
        ParserError::ParserError(
    ...
    let res = all_dialects().parse_sql_statements("SELECT EXISTS (NULL)");
    assert_eq!(
        ParserError::ParserError(
...
fn parse_set_variable() {
    ...
    // parenthesized set variables: removed (no remaining dialects support supports_parenthesized_set_variables)
...
fn ensure_multiple_dialects_are_tested() {
    // Verify that all_dialects() actually contains multiple dialects,
    // so common tests aren't accidentally running against just one.
    let dialects = all_dialects();
    assert!(
        dialects.dialects.len() >= 2,
        "Expected at least 2 dialects, got {}",
...
fn parse_is_boolean() {
    ...
    assert!(matches!(
        verified_expr(sql),
        Expr::IsTrue { expr, .. } if matches!(*expr, Identifier(_))
    ));

    let sql = "a IS NOT TRUE";
    assert!(matches!(
        verified_expr(sql),
        Expr::IsNotTrue { expr, .. } if matches!(*expr, Identifier(_))
    ));

    let sql = "a IS FALSE";
    assert!(matches!(
        verified_expr(sql),
        Expr::IsFalse { expr, .. } if matches!(*expr, Identifier(_))
    ));

    let sql = "a IS NOT FALSE";
    assert!(matches!(
        verified_expr(sql),
        Expr::IsNotFalse { expr, .. } if matches!(*expr, Identifier(_))
    ));
    ...
    assert!(matches!(
        verified_expr(sql),
        Expr::IsUnknown { expr, .. } if matches!(*expr, Identifier(_))
    ));

    let sql = "a IS NOT UNKNOWN";
    assert!(matches!(
        verified_expr(sql),
        Expr::IsNotUnknown { expr, .. } if matches!(*expr, Identifier(_))
    ));
...
fn parse_cache_table() {
    ...
    assert_eq!(
        ...
        Statement::Cache {
            cache_token: AttachedToken::empty(),
            table_flag: None,
...
fn parse_uncache_table() {
    assert_eq!(
        ...
        Statement::UNCache {
            uncache_token: AttachedToken::empty(),
...
fn parse_escaped_string_with_unescape() {
    fn assert_mysql_query_value(dialects: &TestedDialects, sql: &str, quoted: &str) {
        // Use parse_sql_statements directly to avoid roundtrip checks,
        // since MySQL unescaped strings containing control characters
    ...
    // Note: No tested dialects support backslash escape without ignoring wildcard escapes,
    // so we don't test that case here (would result in empty dialect list)
}
...
// parse_projection_trailing_comma: removed (no remaining dialects support supports_projection_trailing_commas)
...
// test_match_recognize and test_match_recognize_patterns: removed (no remaining dialects support supports_match_recognize)
...
// tests_select_values_without_parens: removed (FROM VALUES without parens was only supported by removed dialects: GenericDialect, Snowflake, Databricks)
// tests_select_values_without_parens_and_set_op: removed (same reason)

// parse_select_wildcard_with_except: removed (no remaining dialects support supports_select_wildcard_except)
...
#[test]
fn test_try_convert() {
    // Only MSSQL supports TRY_CONVERT, and it has convert_type_before_value = true.
    let dialects =
    ...
    // No remaining dialects support TRY_CONVERT without convert_type_before_value,
    // so the value-first variant is not tested here.
}
...
fn parse_factorial_operator() {
    ...
    // Note: There are no tested dialects that support bang_not but not factorial,
    // so we skip that test case (would result in empty dialect list)
}
...
fn test_reserved_keywords_for_identifiers() {
    ...
    // Note: Some dialects like Snowflake do not reserve INTERVAL and would allow it as an identifier,
    // but those dialects are not in the tested set, so we don't test that case here
}
...
fn overflow() {
    ...
    let mut statements = Parser::parse_sql(&PostgreSqlDialect {}, sql.as_str()).unwrap();
...
// test_select_from_first: removed (no remaining dialects support supports_from_first_select)
...
fn parse_multiple_set_statements() -> Result<(), ParserError> {
    ...
    match stmt {
        Statement::Set(SetStatement {
            inner: Set::MultipleAssignments { assignments },
            ..
        }) => {
            assert_eq!(
                assignments,
...
fn parse_set_time_zone_alias() {
    match all_dialects().verified_stmt("SET TIME ZONE 'UTC'") {
        Statement::Set(SetStatement {
            inner: Set::SetTimeZone { local, value },
            ..
        }) => {
            assert!(!local);
            assert_eq!(
...
#[test]
fn join_precedence() {
    // Test dialects that support left-associative joins without parentheses
    ...
    // Note: No tested dialects require parentheses for join precedence (would use canonical with parens),
    // so we don't test that case here (would result in empty dialect list)
}
...
fn parse_create_table_like() {
    // Test the plain LIKE syntax (non-parenthesized)
    ...
    // Note: Redshift supports parenthesized LIKE syntax:
    // CREATE TABLE new (LIKE old)
    // CREATE TABLE new (LIKE old INCLUDING DEFAULTS)
    // CREATE TABLE new (LIKE old EXCLUDING DEFAULTS)
    // but Redshift is not in the tested dialects, so we don't test that here
}
...
fn test_parse_set_session_authorization() {
    ...
    assert_eq!(
        ...
        Statement::Set(SetStatement {
            token: AttachedToken::empty(),
            inner: Set::SetSessionAuthorization(SetSessionAuthorizationParam {
