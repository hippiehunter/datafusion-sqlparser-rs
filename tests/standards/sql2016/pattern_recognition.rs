...
fn r021_01_classifier() {
    verified_standard_stmt(
        "SELECT * FROM t \
         MATCH_RECOGNIZE ( \
           ORDER BY ts \
           MEASURES CLASSIFIER() AS matched_var \
           ALL ROWS PER MATCH \
           PATTERN (A | B | C) \
...
fn r030_01_subset() {
    verified_standard_stmt(
        "SELECT * FROM t \
         MATCH_RECOGNIZE ( \
           ORDER BY ts \
           MEASURES SUM(UP.price) AS total_up \
    ...
    verified_standard_stmt(
        "SELECT * FROM events \
         MATCH_RECOGNIZE ( \
           ...
           ONE ROW PER MATCH \
           PATTERN (A+) \
           DEFINE A AS A.event_time - PREV(A.event_time) < INTERVAL '30' MINUTE OR PREV(A.event_time) IS NULL \
         )"
    );
}
