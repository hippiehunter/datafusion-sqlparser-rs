...
fn b200_01_table_function_basic() {
    ...
    verified_standard_stmt(
        "SELECT * FROM TABLE(my_table_function(1, 'test'))"
    );
}
...
fn ptf_type_row_semantics() {
    ...
    verified_standard_stmt(
        "SELECT * FROM TABLE(parse_json(TABLE json_documents ROW SEMANTICS))"
    );
}
...
fn ptf_with_scalar_args() {
    verified_standard_stmt(
        "SELECT * FROM TABLE(sample_rows(TABLE large_table, 0.1, 42))"
    );
}
