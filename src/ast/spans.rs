...
impl Spanned for Statement {
    fn span(&self) -> Span {
        match self {
            ...
            Statement::CreateConversion(create_conversion) => create_conversion.span(),
            Statement::CreateLanguage(create_language) => create_language.span(),
            Statement::CreateEventTrigger(create_event_trigger) => create_event_trigger.span(),
            ...
            Statement::DropAggregate(drop_aggregate) => drop_aggregate.span(),
            ...
            Statement::DropCast(drop_cast) => drop_cast.span(),
            Statement::DropRoutine(drop_routine) => drop_routine.span(),
            Statement::DropTransform(drop_transform) => drop_transform.span(),
            Statement::DropOwned(drop_owned) => drop_owned.span(),
            Statement::ReassignOwned(reassign_owned) => reassign_owned.span(),
...
impl Spanned for UpdateTableFromKind {
    fn span(&self) -> Span {
        let UpdateTableFromKind::AfterSet(from) = self;
